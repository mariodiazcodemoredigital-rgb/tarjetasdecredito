...
## Zona horaria de negocio: México, no UTC crudo (Sprint 47 — bug real crítico, 2026-09-11)

**Problema real reportado por el usuario**: una tarjeta con "Corte día 12" mostraba $0 de utilización (disponible = límite completo) a pesar de tener compras reales registradas, tanto de agosto como de septiembre. Causa raíz: todo el cálculo de "qué ciclo está vigente hoy" (`CreditCardsController.CalcularSaldoNetoAsync`, `PaymentReminderGenerationService.AsegurarRecordatoriosAsync`, `RecommendationController`, `PaymentRemindersController` para "días hasta el límite"/urgencia, `PaymentReminderPushHostedService` para saber si hoy toca mandar un aviso) usaba `DateTime.UtcNow` directo como "hoy". México (`America/Mexico_City`) está en UTC-6 sin horario de verano desde 2022 — así que durante buena parte de la tarde/noche en México, **UTC ya es el día siguiente**. Reproducido y confirmado en vivo: a las 6:31pm del 11 de septiembre (hora de México), el servidor en UTC ya marcaba las 12:31am del **12** de septiembre. Para una tarjeta con `DiaCorte = 12`, eso hacía que `CicloFacturacionHelper.UltimoCorte` calculara el corte como "hoy mismo" (12 de septiembre, según UTC) en vez de "hace un mes" (12 de agosto, la fecha real vigente en México) — excluyendo de golpe **todas** las compras del ciclo real, de agosto y de septiembre por igual.

**Decisión**: ninguna fecha usada para lógica de negocio basada en el **día calendario** (corte de tarjeta, urgencia de vencimiento, disparo de notificaciones push por fecha) debe usar `DateTime.UtcNow` directo — debe pasar primero por `Domain/Reglas/FechaNegocioHelper.AhoraMexico()`, que convierte a la hora de México antes de tomar el `.Date`. Esto NO aplica a timestamps de auditoría/instante puro que nunca se comparan contra un día calendario (ej. `NotificacionOptimaEnviadaUtc`, `FechaPago` capturada como "ahora" — esos siguen en UTC crudo, es correcto que lo estén).

- `FechaNegocioHelper` vive en Domain (no en Infrastructure/Server) porque es una regla de negocio pura, sin dependencias externas — mismo criterio ya usado para `CicloFacturacionHelper`/`FechaTextoHelper`. Resuelve la zona horaria con `TimeZoneInfo.FindSystemTimeZoneById`, probando primero el id de IANA (`America/Mexico_City`, el que existe en Linux/el contenedor Docker de producción) y luego el id de Windows (`Central Standard Time (Mexico)`, para desarrollo local) — si ninguno está disponible (ej. una imagen mínima sin `tzdata`), cae a un offset fijo UTC-6 construido a mano (`TimeZoneInfo.CreateCustomTimeZone`) en vez de fallar, ya que México no observa horario de verano.
- **Sitios corregidos**: `CreditCardsController.CalcularSaldoNetoAsync` (el que causó el bug reportado), `PaymentReminderGenerationService.AsegurarRecordatoriosAsync` (generación de `PaymentReminder` — mismo riesgo de calcular el ciclo equivocado), `RecommendationController.Recomendar` (la fecha por defecto de "¿qué tarjeta uso hoy?"), `PaymentRemindersController.ObtenerTodos`/`MarcarPagado` (el "hoy" usado para `DiasHastaLimite`/urgencia), `PaymentReminderPushHostedService.ProcesarAsync` (el "hoy" usado para decidir si toca mandar un aviso push).
- **Simplificación aceptada a propósito**: la app asume que **todos** los usuarios están en la zona horaria de México — no hay ninguna preferencia de zona horaria por usuario (la app y su copy están en español mexicano, SMTP/dominio `.mx`, todo el diseño asume este público). Si en el futuro se necesitan usuarios en otras zonas horarias, esto tendría que volverse una preferencia por usuario en vez de una constante global.
...
**Regla obligatoria: refrescar `tarjetas` tras cualquier mutación que afecte el saldo (Sprint 42, bug real reportado por el usuario)**: el Server recalcula `Disponible`/`UtilizacionActual` en cada `GET /api/creditcards`, pero el Client los cachea en una lista `tarjetas` que solo se carga una vez (`OnInitializedAsync`). El usuario reportó que, tras registrar una compra en `Compras.razor`, el módulo de Tarjetas mostraba el disponible correcto **pero Compras seguía mostrando el valor viejo** — causa raíz: `GuardarAsync`/`EliminarAsync` de una compra refrescaban la lista de compras (`CargarComprasAsync`) pero nunca volvían a pedir `tarjetas`, así que cualquier componente de esa misma página que dependiera de `Disponible`/`UtilizacionActual` (ej. un `CreditCardVisual` embebido) quedaba desactualizado hasta la siguiente recarga completa de la página. **Regla**: toda página que guarde/edite/elimine una `Purchase` o un `CardPayment` — cualquier operación que cambie `saldoNeto` — debe volver a llamar `CreditCardApiService.ObtenerTodasAsync()` (o el equivalente que recargue `tarjetas`) inmediatamente después, igual que ya recarga la lista que sí editó directamente. No basta con refrescar "lo que se tocó" — hay que refrescar también lo que se deriva de eso.
