...
## Sprint 39 — Mes actual en Pendientes/Próximos, Disponible, máscara de moneda y monto sugerido ✅ (2026-09-11)
- [x] **"Próximos vencimientos" (Dashboard) y "Pendientes" (`/pagos`) restringidos al mes calendario actual**: antes mostraban cualquier ciclo vigente no vencido sin importar el mes de su `FechaLimitePago` (ej. octubre/noviembre apareciendo en septiembre); ahora se filtran también por `FechaLimitePago.Month/Year == hoy`. Implementado como filtro separado de "es próximo" (no fusionado) para no etiquetar por error como "Vencido" algo que solo todavía no le toca — ver [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Próximos vencimientos vs. Historial".
- [x] **`CreditCardDto.Disponible`**: Tarjetas/Dashboard ahora muestran "Disponible $X" junto a "Utilización Y%" en cada tarjeta (`CreditCardVisual.razor`) — `LimiteCredito - saldoNeto`, calculado en el Server junto con la utilización, sin cálculo adicional en el cliente. Documentado en [SPEC-003](docs/specs/SPEC_TarjetasCredito-003-ReglasNegocio.md) "Abonos y utilización neta".
- [x] **Máscara de moneda en vivo**: nuevo componente reutilizable `Components/InputMonto.razor` + `wwwroot/js/currencyMask.js` — separador de miles mientras se escribe (`1000` → `1,000.00`), integrado en Monto de Compras y Monto/Monto del estado de cuenta de Recordatorios. Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Máscara de moneda" (incluye por qué el límite de crédito de `TarjetaFormCard` queda excluido a propósito).
- [x] **Bug real corregido: montos mostrando coma decimal** (`"$10,00"` en vez de `"$10.00"`) — causa raíz: varios `.ToString("N2")`/`ToString("N0")`/`ToString("P0")` sin cultura explícita, sensibles a la cultura del dispositivo del usuario. Corregido forzando `CultureInfo.InvariantCulture` en los 5 sitios encontrados (`Compras.razor`, `Recordatorios.razor` ×3, `CreditCardVisual.razor` ×2). Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Formato de fecha" (sección ampliada para cubrir números, no solo fechas).
- [x] **Monto sugerido de abono en "Ventana óptima de pago"**: la fase 3 de "Mejora tu score" ahora sugiere un rango de abono (o un solo monto si ya está cerca del ideal) calculado contra los umbrales de utilización 10%/30% ya definidos en SPEC-003 — antes solo decía "paga ahora", sin decir cuánto. Documentado en [SPEC-003](docs/specs/SPEC_TarjetasCredito-003-ReglasNegocio.md) "Monto sugerido de abono en Ventana óptima de pago".
- [x] Compila limpio (0 warnings/errores) tras cada cambio.
- [x] Probado de punta a punta en navegador con una cuenta de prueba desechable (creada y eliminada al terminar): dos tarjetas con `DiaCorte`/`DiasParaPago` distintos para forzar una con `FechaLimitePago` en septiembre y otra en octubre — confirmado que Pendientes/Próximos solo muestra la de septiembre; "Disponible" actualizándose en vivo en la vista previa del formulario de alta y correctamente calculado tras registrar una compra ($5,000 límite − $3,000 compra = $2,000 disponible, 60% utilización); máscara de moneda mostrando "3,000" en vivo mientras se escribía "3000" y normalizando a "3,000.00" en blur; historial mostrando "$3,000.00" sin coma decimal; y, forzando fechas en la base local para caer en la ventana óptima, "Mejora tu score" sugiriendo "abona entre $1,500 y $2,500" (verificado a mano: saldo $3,000, 10%→$2,500 a abonar, 30%→$1,500 a abonar, coincide exacto).

## Sprint 40 — Combobox propio, botones con estado claro, editar con foco y fechas con mes completo ✅ (2026-09-11)
- [x] **Categoría de compra: combobox propio en vez de `<datalist>` nativo**: causa raíz del reporte "no está creando las categorías dinámicamente" — el guardado/consulta de categorías ya funcionaba bien (Sprint 37), pero `<datalist>` tiene soporte pobre/inconsistente en navegadores móviles (Safari iOS en particular), así que al usuario nunca le aparecía ningún desplegable. Nuevo `Components/InputCombo.razor` (sin JS, con el truco `@onmousedown:preventDefault` para que el clic en una sugerencia no le quite el foco al input antes de registrar la selección) reemplaza al datalist en `Compras.razor`. Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Combobox de texto libre con sugerencias".
- [x] **Botones "Registrar compra"/"Recomendar tarjeta" con estado deshabilitado claramente distinto**: antes solo bajaba la opacidad del mismo degradado (`opacity:0.6`), muy sutil; ahora deshabilitado usa un estilo plano gris (`var(--surface-2)`) sin degradado ni brillo — inconfundible contra el estado habilitado. Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Botón deshabilitado con estilo plano".
- [x] **Editar desplaza el foco al formulario y resalta el ítem**: en Compras y Abonos, hacer clic en ✏️ ahora hace scroll suave hasta el formulario (`wwwroot/js/domUtils.js`) y resalta la fila que se está editando (`.item-en-edicion`) hasta guardar o cancelar. Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Editar desplaza el foco al formulario y resalta el ítem".
- [x] **Monto de la compra del Dashboard con máscara de moneda y sin "0" fantasma**: nuevo helper `Services/MontoFormato` (variante "en vivo" en C#, sin JS) — a diferencia de `InputMonto` (que sincroniza solo en blur), este formatea en cada tecla porque el botón "Recomendar tarjeta" necesita reaccionar mientras se escribe, no solo al salir del campo. Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Máscara de moneda" (sección ampliada).
- [x] **Fechas con nombre de mes completo, sin abreviar**: `FechaFormato.Corta` pasó de `"10 Sep 2026"` a `"03 Octubre 2026"`; nueva `FechaFormato.CortaSinAnio` (`"12 Septiembre"`) reemplaza los formatos `dd/MM` con barras que usaban los mensajes de "Mejora tu score". El texto de explicación de "¿Qué tarjeta uso hoy?" se arma en Domain (server-side) y no puede depender de Client, así que se creó `Domain/Reglas/FechaTextoHelper.cs` — una copia intencional de la misma tabla de meses en la capa correcta (ver SPEC-001, Domain no depende de Client). También corregido en `Configuracion.razor` (fecha de alta de Face ID). Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Formato de fecha".
- [x] Compila limpio (0 warnings/errores).
- [x] Probado de punta a punta en navegador con una cuenta de prueba desechable (creada y eliminada al terminar): combobox de categoría filtrando en vivo, seleccionando "Restaurantes" con clic, escribiendo la categoría nueva "Gimnasio" y confirmando que aparece como sugerencia al volver a abrir el formulario (el bug original reportado); botón "Registrar compra" pasando de gris plano a degradado vivo en cuanto el formulario queda completo; clic en ✏️ haciendo scroll hasta el formulario y resaltando la fila (`.item-en-edicion` confirmado por inspección del DOM); campo de monto del Dashboard mostrando "$2,500" con coma en vivo mientras se escribía y el botón "Recomendar tarjeta" encendiéndose al instante; y el texto de recomendación generado en el servidor mostrando "21 Octubre 2026" y "Mejora tu score" mostrando "01 Octubre"/"28 Septiembre" (sin abreviar, sin barras).

## Sprint 41 — Últimos 4 dígitos numérico, límite de crédito con máscara, historial por tarjeta ✅ (2026-09-11)
- [x] **Últimos 4 dígitos solo acepta números**: antes aceptaba cualquier texto — filtro `char.IsDigit` en `@oninput`, teclado táctil numérico (`inputmode="numeric"`). La validación real (rechazar si no son 4 dígitos) ya vivía en el Server sin cambios.
- [x] **Límite de crédito con máscara de moneda y sin "0" fantasma**: reutiliza `Services/MontoFormato` (la variante "en vivo" del Sprint 40) en vez de `InputMonto`, porque este campo necesita reactividad por tecla para la vista previa en vivo de la tarjeta — mismo criterio ya documentado para el monto del Dashboard.
- [x] **Historial de Compras agrupado por tarjeta**: un panel por tarjeta activa (encabezado = `CreditCardVisual` en solo lectura, igual que en `/tarjetas`) con sus compras debajo, en una grilla responsiva de 2 columnas desde escritorio (`min-width:992px`) y 1 columna en móvil — con 1 sola tarjeta, ocupa solo la primera celda (comportamiento nativo de CSS Grid, sin código especial).
- [x] Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md): "Alta y edición de tarjeta" ampliada (últimos 4 dígitos + límite de crédito) y nueva sección "Historial agrupado por tarjeta, en grilla responsiva de 2 columnas".
- [x] Compila limpio (0 warnings/errores).
- [x] Probado de punta a punta en navegador con una cuenta de prueba desechable (creada y eliminada al terminar) y 3 tarjetas registradas: "ab12cd" en Últimos 4 dígitos se filtró a "12" (el Server rechazó correctamente el envío con menos de 4 dígitos, confirmando que la validación real sigue viva); Límite de crédito mostrando "$15,000"/"$8,000" en vivo mientras se escribía; grilla de escritorio confirmada por geometría real del DOM (`getBoundingClientRect`): tarjetas A y B en la misma fila (columnas 1 y 2), tarjeta C cayendo sola a la fila siguiente en la columna 1; vista móvil (375px) confirmada con las 3 en una sola columna apiladas; y una compra registrada en Tarjeta A apareciendo dentro de su panel mientras B y C mantenían su "Aún no hay compras en esta tarjeta."

## Sprint 42 — Bugs reportados en Compras: disponible desactualizado, monto que se traba, combo de tarjeta ✅ (2026-09-11)
- [x] **Bug real: "Registrar compra" se quedaba deshabilitado mientras se escribía el monto**, solo se arreglaba borrando decimales — causa raíz encontrada: `InputMonto` (Sprint 38) bindeaba el `value` del input directo al parámetro `Value`, así que cualquier re-render del padre (no hacía falta que fuera del propio campo) reescribía el campo con el valor viejo, peleando contra la máscara JS y a veces dejando un valor que `decimal.TryParse` ya no podía leer. Reescrito para usar `Services/MontoFormato` (C#, sin JS, mismo mecanismo ya probado en el Dashboard) con un campo local `_texto` que solo se resincroniza desde `Value` cuando cambió por una causa externa — nunca mientras el usuario está escribiendo. `wwwroot/js/currencyMask.js` se eliminó (ya no lo usa nadie).
- [x] **Bug real: el disponible de una tarjeta no se actualizaba dentro de Compras** al registrar/editar/eliminar una compra (sí se veía correcto en el módulo de Tarjetas) — causa raíz: `Compras.razor` nunca volvía a pedir `tarjetas` después de guardar, solo refrescaba la lista de compras. Agregado `CargarTarjetasAsync()` tras cada guardado/eliminación exitosos. Nueva regla general documentada en [SPEC-003](docs/specs/SPEC_TarjetasCredito-003-ReglasNegocio.md) "Abonos y utilización neta".
- [x] **Historial de Compras: la grilla de 2 columnas por tarjeta del Sprint 41 se reemplazó por un combo** ("Tarjeta a consultar") con opción "Ver todos" (todas las compras, fecha descendente) más una opción por tarjeta (filtra a solo esa). La intención original del usuario en el Sprint 41 nunca fue una grilla siempre visible, sino un selector — aclarado en esta solicitud. Documentado en [SPEC-005](docs/specs/SPEC_TarjetasCredito-005-UI.md) "Historial de compras: combo para elegir tarjeta".
- [x] Compila limpio (0 warnings/errores).
- [x] Probado de punta a punta en navegador con una cuenta de prueba desechable (creada y eliminada al terminar) y 2 tarjetas: escribí "1250.9999" en Monto y se truncó correctamente a "1,250.99" en vivo, con el botón "Registrar compra" habilitándose de inmediato (sin necesitar salir del campo) y sobreviviendo intacto a un re-render posterior (clic + escritura en Descripción) — el bug original ya no reproduce. Confirmé por red (`GET /api/creditcards`) que tras guardar la compra el disponible de "Tarjeta Dos" regresó ya actualizado ($6,000 − $1,250.99 = $4,749.01), y que el módulo de Tarjetas mostró el mismo valor. Probé el combo del historial: "Ver todos" mostró ambas compras (de las 2 tarjetas), y al elegir "Tarjeta Dos" filtró a solo esa.

## Sprint 43 — Historial de Compras: filtro de periodo y paginación ✅ código, pendiente verificación en navegador (2026-09-11)
- [x] Nuevo `<select>` "Periodo" en `Compras.razor` (combinable con el de tarjeta ya existente): "Mes actual" (por defecto) o "Último corte" — este último calculado por tarjeta (`Services/CicloFacturacionCliente.UltimoCorte`, duplicado de `Domain/Reglas/CicloFacturacionHelper.UltimoCorte` por la misma razón que `FechaTextoHelper`).
